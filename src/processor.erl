...
calculate_grammsAtMonth(Products, {Calories, FPC, Minerals, Vitamins}, Budget) ->
    ...
    ProductIndexses = [Indexses || [_Name, _EnName | Indexses] <- Products],
    Coefficients = lists:flatten([Calories, Budget, FPC, MineralsCoeff, VitaminsCoeff]), 
    Masses = matlab:solve_equations(ProductIndexses, Coefficients),
    [ #product{name = Name, ccal = CCal, price = Price, mass = Mass} || Mass <- Masses, [Name, _EnName, CCal, Price | _Tail] <- Products].
